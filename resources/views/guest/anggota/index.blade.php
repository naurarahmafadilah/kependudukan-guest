...
@endsection
  ...
  <form method="GET" action="{{ route('anggota-keluarga.index') }}" class="row g-2 align-items-end mb-4">
    {{-- Filter hubungan --}}
    <div class="col-md-2">
    ...
    {{-- Search (panjangnya sama kayak Warga & KK: col-md-4) --}}
    <div class="col-md-4">
      <div class="input-group">
        <input
          type="text"
          name="search"
          class="form-control"
          placeholder="Cari nama, No KTP, atau No KK"
          value="{{ request('search') }}">
  ...
  <div class="row g-4">
    ...
    <div class="col-md-4 col-sm-6">
      <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden">
        ...
        <div class="card-footer bg-light border-0 d-flex justify-content-between px-4 py-3">
          <a href="{{ route('anggota-keluarga.show', $item) }}" class="btn btn-sm btn-info text-white">
            <i class="bi bi-eye"></i> Detail
          </a>
          ...
          <form action="{{ route('anggota-keluarga.destroy', $item) }}" method="POST" class="d-inline">
            @csrf
            @method('DELETE')
            <button class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus data anggota ini?')">
              <i class="bi bi-trash"></i> Hapus
            </button>
          </form>
        </div>
